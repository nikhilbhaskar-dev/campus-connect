...
import { LogOut, Navigation, Star, CheckCircle, Clock } from 'lucide-react';
import { MapContainer, TileLayer, Marker, Popup, useMap } from 'react-leaflet';
import L from 'leaflet';
import 'leaflet/dist/leaflet.css';

const availableRidesMock = [
  { id: 101, passengerName: "Aanya S.", pickup: "Main Library", destination: "Hostel C Gate", distance: "1.3 km", fare: 22, eta: "2 min", coords: [29.866, 77.896] },
  { id: 102, passengerName: "Kabir D.", pickup: "Cafeteria", destination: "Main Gate", distance: "2.1 km", fare: 35, eta: "5 min", coords: [29.862, 77.892] }
];
...
const weeklyStatsMock = {
  ...
  totalEarnings: "3,840",
  chartData: [
    { day: 'M', value: 5, active: false },
    { day: 'T', value: 8, active: false },
    { day: 'W', value: 12, active: true },
    { day: 'T', value: 7, active: false },
    { day: 'F', value: 10, active: false },
    { day: 'S', value: 14, active: true },
    { day: 'S', value: 4, active: false },
  ]
};
...
export default function DriverDashboard() {
  ...
  const toggleOnlineStatus = () => {
    ...
    if (newStatus && navigator.geolocation) {
      navigator.geolocation.getCurrentPosition((pos) => setDriverCoords([pos.coords.latitude, pos.coords.longitude]));
    }
  };
  ...
  return (
    <div className="min-h-screen pb-20 bg-paper">
      <nav className="flex items-center justify-between px-6 py-5 max-w-[1100px] mx-auto">
        ...
        <button onClick={() => { localStorage.clear(); navigate('/'); }} className="w-9 h-9 flex items-center justify-center rounded-full border border-line-soft hover:bg-line hover:text-paper text-ink transition-all"><LogOut size={16} /></button>
      </nav>
      ...
      <header className="px-6 pt-6 pb-6 max-w-[1100px] mx-auto flex flex-wrap justify-between items-center gap-4 border-b border-line-soft mb-8">
        ...
        <button onClick={toggleOnlineStatus} className={`relative flex items-center w-36 h-12 rounded-full p-1 transition-colors ${isOnline ? 'bg-line' : 'bg-white border-2 border-line-soft'}`}>
          <div className={`absolute w-10 h-10 rounded-full flex items-center justify-center transition-transform ${isOnline ? 'translate-x-24 bg-white' : 'translate-x-0 bg-paper border border-line-soft'}`}>
      ...
      {activeTab === 'requests' ? (
          ...
          {!isOnline ? (
            ...
            <>
              <div className="flex flex-col gap-4">
                {availableRidesMock.map((req) => (
                  <div key={req.id} className="bg-white border border-line-soft rounded-[14px] p-6 shadow-sm">
                    <div className="flex justify-between mb-6">
                      <div><div className="font-bold">{req.passengerName}</div><div className="text-xs text-grey">{req.eta} away</div></div>
                      <div className="text-right font-bold text-xl">₹{req.fare}</div>
                    </div>
                    <div className="relative pl-6 mb-6 text-sm"><div className="absolute left-[3px] top-1 bottom-1 w-[2px] bg-line-soft"></div><div className="mb-4">{req.pickup}</div><div>{req.destination}</div></div>
                    <button className="w-full bg-line text-paper font-bold py-3 rounded-full">Accept Ride</button>
                  </div>
                ))}
              </div>
              <div className="h-[500px] bg-white border border-line-soft rounded-[14px] p-2">
                <MapContainer center={driverCoords} zoom={15} style={{ height: '100%', width: '100%' }}>
                  ...
                  {availableRidesMock.map(req => <Marker key={req.id} position={req.coords} icon={requestIcon} />)}
                </MapContainer>
              </div>
            </>
          )}
        ...
        <div className="px-6 max-w-[800px] mx-auto space-y-6">
          ...
          <div className="bg-white border border-line-soft rounded-[20px] p-8">
            <h3 className="font-bold text-lg mb-6">Full Ride History</h3>
            {historyMock.map(ride => (
              <div key={ride.id} className="flex justify-between py-4 border-b last:border-0">
                <div><div className="font-bold">{ride.pickup} → {ride.destination}</div><div className="text-sm text-grey">{ride.passengerName}</div></div>
                <div className="text-right font-bold text-line">+₹{ride.earnings}</div>
              </div>
            ))}
          </div>
        </div>
